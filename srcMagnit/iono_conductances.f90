...
subroutine FACs_to_fluxes(iModel, iBlock)
  ...
  real, dimension(IONO_nTheta,IONO_nPsi) :: nDen, &
       ...
       ion_ave_e, iono_Ni, iono_Ti, ion_eflux, IONO_NORTH_IDIF_NFlux, IONO_SOUTH_IDIF_NFlux, &
       poynting, IONO_NORTH_BBND_NFlux, IONO_SOUTH_BBND_NFlux, IONO_NORTH_MONO_NFlux, &
       IONO_SOUTH_MONO_NFlux
  ...
  real :: MulFac_Dae, MulFac_Def, var_rm, var_tot, numerator, denominator, &
       rm, vari, numflux_floor, k, DIFF_LCF1, MONO_LCF1, IDIF_LCF1, &
       DIFF_LCF2, MONO_LCF2, IDIF_LCF2, DIFF_LCF3, MONO_LCF3, IDIF_LCF3, &
       BBND_LCF1, BBND_LCF2, BBND_LCF3
  ! real, dimension(IONO_nTheta,IONO_nPsi) :: nDen, &
  !     discrete_k, discrete_ae, discrete_ef, diffuse_ae, diffuse_ef, &
  ...
  if (iModel == 11) then ! ADELPHI AMPERE-derived model
  ...
  if (iModel == 4 .or. iModel == 5 .or. iModel == 9) then
...
!     MulFac_ae = 1.0 / 1.0e11
     ...
     ! Loss Cone Factors                                                       
     DIFF_LCF1 = 0.217 ! For Energy Flux                                       
     MONO_LCF1 = 1.0
     IDIF_LCF1 = 0.207
     BBND_LCF1 = 2.247

     DIFF_LCF2 = 0.055 ! For Number Flux
     MONO_LCF2 = 0.741
     IDIF_LCF2 = 0.038
     BBND_LCF2 = 0.494

     DIFF_LCF3 = DIFF_LCF1/DIFF_LCF2 ! For Average Energy
     MONO_LCF3 = MONO_LCF1/MONO_LCF2
     IDIF_LCF3 = IDIF_LCF1/IDIF_LCF2
     BBND_LCF3 = (BBND_LCF1**0.5)/(BBND_LCF2**0.47)
...
!           enddo
        ...
        ! =============== SOURCE ==================
        ! Proton Diffuse Precipitation Calculation:
        ! =========================================
        !
        ! Following similar procedure as in Gilson et al. 2012;
        ! We already have a good estimate of the average energy based on
        ! the MHD ion temperature. In the following section, we find
        ! E-flux again!
        
        ! Number Flux = N_e * ((kB * T)/(2pi * m_ion))**0.5
        ! N_e = Density conversion (amu/cm3) = 1.66E-21 kg/m3
        ! T = Ave_E * 1E03 * 11000 (conversion from keV to Kelvins)
        ! m_ion = 1.67E-27 kg
        ! kB = 1.38E-23 J/K
        ! Multiplication Factor = (kB/(2pi * m_ion))**0.5
        !                       = 36.26531
        ...
        ! Energy Flux = Number Flux * Average Energy
        ! MulFac_ae is for electrons which assumes 1/6 temperature for the
        ! electrons. For ions, this would be 5/6. Therefore, the average
        ! energy is multiplied by 5.
        ion_ave_e = &
                iono_north_p / (iono_north_rho + 1.0e-32) * MulFac_ae * 5.
        
        ! Conversion to amu/cm3
        iono_Ni = iono_north_rho / 1.66e-21
        
        !write(*,*) 'MaxVal Number Density (in amu/cm3)', maxval(iono_Ne)
        ! Conversion to Kelvins
        iono_Ti = ion_ave_e * 1e03 * 1.1e04
        !write(*,*) 'N_Diff: MaxVal Ave_E (in keV)', maxval(iono_north_ave_e)
        !write(*,*) 'MaxVal Temperature (in K)', maxval(iono_T)
        ! Energy Flux
        ion_eflux = iono_Ni * 1.38e-23 * 1e06 * &
             36.26531 * iono_Ti**1.5 !* &
             !IDIF_LCF1! Beta = Loss Cone Factor (Can be replaced using same method as in RCM)
        !write(*,*) 'N_Diff: MaxVal EFlux (in W/m2)', maxval(iono_north_eflux)

        IONO_NORTH_IDIF_NFlux = iono_Ni * 1e06 * 36.26531 *&
             iono_Ti**0.5 * IDIF_LCF2
        ...
        if (DoTest) then
           write(*,*) 'N_Ion: MaxVal Ave_E (in keV)',  maxval(ion_ave_e)
           write(*,*) 'N_Ion: MaxVal EFlux (in W/m2)', maxval(ion_eflux)
        end if
           
        ! Transfer to Global Variables
        IONO_NORTH_IDIF_AVE_E = ion_ave_e * IDIF_LCF3
        IONO_NORTH_IDIF_EFLUX = ion_eflux * IDIF_LCF1

        ! ============ SOURCE ================
        ! Broadband Precipitation Calculation:
        ! ====================================
        !
        ! Following similar procedure as in Zhang et al. 2015;
        ! Using the empirical relationship in the aforementioned
        ! publication, the Joule Heating is used to derive
        ! Poynting Flux which is used to empirically derive
        ! number and energy flux, and average energy.
        
        ! Number Flux = 3E09 * (S_|| * A) ** 0.5
        ! Energy Flux = 2 * (S_|| * A) ** 0.47
        ! Ave. E = 0.41 * (S_|| * A) ** 0.03

        ! Multiplication Factor = 0.43395593979143521
        ! (Multiplication Factor from GM - derived at 4000 km)
        ! This value should be formalized further as I just copy-pasted
        ! this from BATSRUS SI2Unit_ (in ) - AMH.
        ! --- Under Construction ---
        
        ! Note: Joule Heating is in SI Units = W/m2
        poynting = IONO_NORTH_Joule * 0.43395593979143521 ! in W/m2

        ! A = 2.5 - To be formalized into PARAMs....
        ! --- Under Construction ---
        IONO_NORTH_BBND_EFLUX = 2 * (poynting * BBND_LCF1)**0.5 ! in mW/m2 or ergs/cm2 
        IONO_NORTH_BBND_NFlux = 3E09 * (poynting * BBND_LCF2)**0.47 ! in particles/cm2.s
        IONO_NORTH_BBND_AVE_E = 0.41 * (poynting)**0.03 * BBND_LCF3 ! in keV
        
        ! Convert E-Flux to W/m2
        IONO_NORTH_BBND_EFLUX = IONO_NORTH_BBND_EFLUX/1E03

        if (DoTest) then
           write(*,*) 'N_BBd: MaxVal Poynt (in W/m2)', maxval(poynting/1E03)
           write(*,*) 'N_BBd: MaxVal Ave_E (in keV)',  maxval(IONO_NORTH_BBND_AVE_E)
           write(*,*) 'N_BBd: MaxVal EFlux (in W/m2)', maxval(IONO_NORTH_BBND_EFLUX)
        end if
        ...
        do j = 1, IONO_nPsi
           ...
           do i = 1, IONO_nTheta - 1
              ...
              discrete_nf(i,j) = discrete_nf(i,j)
        ...
        IONO_NORTH_MONO_NFlux = discrete_nf * MONO_LCF2
        diffuse_nf = iono_Ne * 1e06 * 1553.5632 * iono_T**0.5
        ...
        iono_north_ave_e =                & ! If using both precip
             (diffuse_ef*DIFF_LCF1 + discrete_ef*MONO_LCF1) / &
             (1E03 * 1.6e-19 *            & ! Convert this into keV (x 10^3)
             (diffuse_nf*DIFF_LCF2 + discrete_nf*MONO_LCF2))
        ...
        IONO_NORTH_DIFF_Ave_E = diffuse_ae * DIFF_LCF3
        IONO_NORTH_DIFF_EFlux = diffuse_ef * DIFF_LCF1

        IONO_NORTH_MONO_Ave_E = discrete_ae * MONO_LCF3
        IONO_NORTH_MONO_EFlux = discrete_ef * MONO_LCF1
        ...
        ! Energy Flux = Number Flux * Average Energy
        ion_ave_e = &
                iono_south_p / (iono_south_rho + 1.0e-32) * MulFac_ae * 5.
        
        ! Conversion to amu/cm3
        iono_Ni = iono_south_rho / 1.66e-21
        ...
        IONO_SOUTH_IDIF_AVE_E = ion_ave_e * IDIF_LCF3
        IONO_SOUTH_IDIF_EFLUX = ion_eflux * IDIF_LCF1
        ...
        ! (Multiplication Factor from GM - derived at 4000 km)
        ! This value should be formalized further as I just copy-pasted
        ! this from BATSRUS SI2Unit_ - AMH.

        ! Note: Joule Heating is in SI Units = W/m2
        poynting = IONO_SOUTH_Joule * 0.43395593979143521 ! in W/m2

        ! A = 2.5 - To be formalized into PARAMs....
        IONO_SOUTH_BBND_EFLUX = 2 * (poynting * BBND_LCF1)**0.5 ! in mW/m2 or ergs/cm2
        IONO_SOUTH_BBND_NFlux = 3E09 * (poynting * BBND_LCF2)**0.47 ! in particles/cm2.s
        IONO_SOUTH_BBND_AVE_E = 0.41 * (poynting)**0.03 * BBND_LCF3 ! in keV
        
        ! Convert E-Flux to W/m2
        IONO_SOUTH_BBND_EFLUX = IONO_SOUTH_BBND_EFLUX/1E03

        if (DoTest) then
           write(*,*) 'S_BBd: MaxVal Poynt (in W/m2)', maxval(poynting/1E03)
           write(*,*) 'S_BBd: MaxVal Ave_E (in keV)',  maxval(IONO_SOUTH_BBND_AVE_E)
           write(*,*) 'S_BBd: MaxVal EFlux (in W/m2)', maxval(IONO_SOUTH_BBND_EFLUX)
        end if
        ...
        ! For Storage
        IONO_SOUTH_DIFF_Ave_E = diffuse_ae * DIFF_LCF3
        IONO_SOUTH_DIFF_EFlux = diffuse_ef * DIFF_LCF1

        IONO_SOUTH_MONO_Ave_E = discrete_ae * MONO_LCF3
        IONO_SOUTH_MONO_EFlux = discrete_ef * MONO_LCF1
...
subroutine ionosphere_conductance(Sigma0, SigmaH, SigmaP, &
  ...
  real, dimension(1:IONO_nTheta,1:IONO_nPsi) ::  &
       ...
       Eflux, Ave_E, I_Eflux, I_Ave_E, BBnd_Eflux, BBnd_Ave_E,  &
  ...
  real :: SigmaH_Particles, SigmaP_Particles, tau, SigmaH_Ion, &
       SigmaP_Ion, SigmaH_BBd, SigmaP_BBd
  logical :: old
  ...
  if (use_comp(UA_)) then
     ...
     if ((iModel >= 3) .and. (iModel /= 10)) then
        ...
        do j = 1, nPsi
           do i = 1, nTheta
              ...
              SigmaP_Ion = 5.7  *                     &
                   sqrt(I_EFlux(i,j)*1000.0)

              SigmaH_Ion = 0.45 * (I_Ave_E(i,j)**0.3) * SigmaP_Ion
              
              ! Add Broadband Precipitation as stipulated in
              ! Zhang et al. (2015) - Not vector addition.
              SigmaP_BBd = 40.0 * BBnd_Ave_E(i,j) /             &
                   (16.0 + BBnd_Ave_E(i,j)*BBnd_Ave_E(i,j))  *          &
                   sqrt(BBnd_EFlux(i,j)*1000.0)
              
              SigmaH_BBd = 0.45 * (BBnd_Ave_E(i,j)**0.85) * &
                   SigmaP_BBd
              
              ! This is an extremely stupid way to do the total conductance. - AM
              SigmaH(i,j) = sqrt(SigmaH_EUV*SigmaH_EUV + &
                   SigmaH_SCAT*SigmaH_SCAT + &
                   SigmaH_STAR*SigmaH_STAR + &
                   SigmaH_Particles*SigmaH_Particles + &
                   SigmaH_Ion*SigmaH_Ion) + SigmaH_BBd

              ! (Repeated for extra effect)
              ! This is an extremely stupid way to do the total conductance. - AM
              SigmaP_Ion = SigmaP_Ion * SigmaP_Ion
...
subroutine determine_oval_characteristics(Current_in, Theta_in, Psi_in, &
  ...
  do n = 1, 8
     ...
     if (width(n) <= (theta(2,1)-theta(1,1))) width(n) = max_fac_colat(n)/5.0
